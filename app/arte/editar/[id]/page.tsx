...
export default function EditarArtePage() {
  ...
  useEffect(() => {
    const fetchArte = async () => {
      ...
      try {
        ...
        const tipo = arteData.tecnicaEscultorica ? 'escultura' : 'cuadro';
        setTipoObra(tipo);
        
        // Rellenar el formulario con los datos de la obra
        setFormData({
          id: arteData.id || "",
          titulo: arteData.titulo || "",
          autor: arteData.autor || "",
          tematica: arteData.tematica || "",
          tecnicaPictorica: arteData.tecnicaPictorica || "",
          tecnicaEscultorica: arteData.tecnicaEscultorica || "",
          certificado: arteData.certificado || false,
        ...
        setError('Error al cargar los datos de la obra de arte');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        let errorMessage = "Error al actualizar la obra de arte";
        try {
          const errorData = await res.json();
          console.error("Error del servidor:", errorData);
          errorMessage = errorData.detail || errorData.message || JSON.stringify(errorData);
        } catch (jsonError) {
          console.error("No se pudo parsear el error:", jsonError);
      ...
      if (error instanceof Error) {
        ...
        setError("Error desconocido al actualizar la obra de arte");
      }
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="flex justify-center items-center h-64">
          <div className="text-lg text-gray-600">Cargando datos de la obra de arte...</div>
        </div>
      </div>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <ul className="text-xs text-gray-600 space-y-1">
              ...
              <li>• Las medidas se especifican en centímetros (cm)</li>
          ...
          <div className="flex justify-end pt-4">
            ...
            >
              <Save size={20} />
              {loading ? "Actualizando..." : "Actualizar Obra"}
            </button>
          </div>
        </form>
      </div>
